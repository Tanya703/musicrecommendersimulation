...
MoodLense 1.0 was evaluated against three baselines: **No-div** (same algorithm, no diversity penalty), **Binary** (exact-match mood/genre, no partial credit), and **Random** (sanity-check floor). Results across 11 user profiles:

*Auto-generated by `python -m src.evaluate` — re-run to refresh.*

<!-- EVAL:START -->
*Run `python -m src.evaluate` to populate this section.*
<!-- EVAL:END -->
